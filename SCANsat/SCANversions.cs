	...
	[KSPAddon (KSPAddon.Startup.MainMenu, true)]
		...
		private void findAssemblies ( string[] assemblies ) {
			...
			foreach (string name in assemblies) { //Search for the relevant plugins among the loaded assemblies
				...
				if (assembly != null) assemblyList.Add (new AssemblyLog (assembly));
			}
		...
		private void debugWriter () {
			foreach (AssemblyLog log in assemblyList) {
				print (string.Format ("[SCANversion] Assembly: {0} found; Version: {1}; File Version: {2}; Info Version: {3}; Location: {4}", log.name, log.version, log.fileVersion, log.infoVersion, log.location));
			}
		}
	}

	//A class to gather and store information about assemblies
	//[DebuggerDisplayAttribute("Name -> {name}, Version -> {version}, InfoVersion -> {infoVersion}, Location -> {location}'")]
